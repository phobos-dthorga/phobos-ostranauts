...
Framework supplies shared services rather than a set of standalone machines. Its shared repair remainder can appear while servicing equipment from any participating Phobos content mod. Construction menus, machinery controls and the opt-in Scope recorder are interfaces, not additional inventory objects. Crew standing orders are opt-in; see [crew work, training and time-skips](crew-automation.md) for eligible work, approved stores, resource limits and explicit industrial/exterior Resume. Compact controls, configuration drafts and object selection are explained in the [control-panel guide](control-panel-guide.md). In selection fields, Locate opens a temporary marked ship view; Clear is a draft change that requires Apply. Pick on ship marks eligible objects and Escape restores the panel and view.
