...
**Prepared versions:** Phobos Framework **0.26.1**, Shipbreaker **0.26.1**, Auto Nav
**0.20.0**, built against Ostranauts **1.0.1.5** / BepInEx **5.4.23.5**.
...
[Console and equipment panel guide](industrial-console-player-guide.md): a 3 x 3 ship-bound workstation, local Control Panels, automatic grouping, search, Attention and routing. The current Shipbreaker 0.26.1 requires Framework 0.26.1 and Auto Nav 0.19.0 and includes [shared observations](shared-console-observations.md) and optional [shared completion cues](shared-completion-cues.md). Prepared for owner testing; no in-game validation claimed.
