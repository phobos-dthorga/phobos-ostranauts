...
internal static class Content
{
    ...
    internal static void Register(Action<string> log)
    {
        ...
        try
        {
            var problems = WorkshopAdapter.Inspect(out string versions);
            DependencyStatus = versions + "\n" + (problems.Count == 0 ? "Template precheck passed." : string.Join("\n", problems));
            if (problems.Count != 0)
            {
            ...
            var prepared = new PreparedDefinitions();
            ...
            foreach (string variant in new[] { Installed, Loose, Installed + "Dmg", Loose + "Dmg" })
            {
                ...
                co.strDesc = "4 x 4 industrial fixture; 160 kg. Feed up to four ordinary loose wall panels. " +
                    Plugin.Options.ControlsKey + " opens controls while nearby. A new batch takes " + Plugin.Options.CycleSeconds +
                    " seconds at " + Plugin.Options.WorkingKW + " kW and recovers 11 kg of parts/scrap " +
                ...
                // A full-size native work deck is a temporary runtime reference, never packaged artwork.
                var deck = DataHandler.dictItemDefs["ItmFloorGrate4x401"];
                item.strImg = deck.strImg; item.strImgNorm = deck.strImgNorm;
                item.strImgDamaged = deck.strImg;
                co.strPortraitImg = deck.strImg;
                item.nCols = ProcessRules.Footprint;
                bool installed = variant.StartsWith(Installed, StringComparison.Ordinal);
                item.aSocketAdds = Enumerable.Repeat(installed ? "TILFixtureAdds" : "TILItemAdds", 16).ToArray();
            ...
            power.fAmount = Plugin.Options.IdleKW / 3600;
            power.strOverrideCond = ProcessRules.Working;
            power.fOverrideAmount = Plugin.Options.WorkingKW / 3600;

            RegisterAssemblySection(prepared);
            var residue = WorkshopAdapter.Clone(DataHandler.dictCOs["ItmScrapTrash"]);
            residue.strName = ProcessRules.Residue;
    ...
    internal static void ConfirmRecipes(Action<string> log)
    {
        ...
        DependencyStatus += "\n" + (Ready ? "Both construction recipes registered. In-game compatibility remains unverified."
            : string.Join("\n", missing) + "\nInspect the Crafting Framework log and matching Phobos plugin/data package.");
        log(Status + "\n" + DependencyStatus);
    }

    private static void RegisterAssemblySection(PreparedDefinitions prepared)
    {
        // Two tangible 80 kg sections keep every craft within OCF's 100-unit
        // input limit without reducing the full machine's mass or material bill.
        var section = WorkshopAdapter.Clone(DataHandler.dictCOs["ItmScrapSteel"]);
        var item = WorkshopAdapter.Clone(DataHandler.dictItemDefs[section.strItemDef]);
        var deck = DataHandler.dictItemDefs["ItmFloorGrate4x401"];
        ...
        section.strDesc = "An unpowered 80 kg, 4 x 4 assembly section. Combine two at a Salvage Workshop " +
            "workbench to finish one powered dismantling fixture. Not ordinary scrap; cannot process panels by itself.";
        section.inventoryWidth = section.inventoryHeight = ProcessRules.Footprint;
        section.nStackLimit = 1;
        section.strPortraitImg = deck.strImg;
