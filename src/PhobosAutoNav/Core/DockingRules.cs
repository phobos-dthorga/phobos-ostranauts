...
internal static class DockingRules
{
    ...
    internal const double StableMotionSeconds = 5, TerminalAuthorityShare = .35, MaximumResidualMS = .05;
    ...
    internal static bool TryGuide(double dx, double dy, double vx, double vy, double rotation, double spin,
        double contactM, double acceleration, double throttle, double dt, out DockingCommand command, NavVector targetAcceleration = default, double facingOffset = 0)
    {
        command = default;
        ...
        if (!targetAcceleration.Finite || !ArrivalBrake.Finite(facingOffset)) return false;
        ...
        double heading = Wrap(-Math.Atan2(dx, dy) - rotation - facingOffset);
