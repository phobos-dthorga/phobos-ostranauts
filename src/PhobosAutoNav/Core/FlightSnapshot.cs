...
internal enum SavedFlightMode { Active, Suspended, Stopped, Arrived, Docking, DockingSuspended, Docked, Rendezvous, RendezvousSuspended, Following, FollowingSuspended, ApproachDock, ApproachDockSuspended }
...
internal sealed class FlightSnapshot
{
    ...
    internal bool IsCombinedApproach => Mode == SavedFlightMode.ApproachDock || Mode == SavedFlightMode.ApproachDockSuspended;
    ...
    internal bool IsActive => Mode == SavedFlightMode.Active || Mode == SavedFlightMode.Docking || Mode == SavedFlightMode.Rendezvous || Mode == SavedFlightMode.Following || Mode == SavedFlightMode.ApproachDock;
    internal bool IsResumable => IsActive || Mode == SavedFlightMode.Suspended || Mode == SavedFlightMode.DockingSuspended || Mode == SavedFlightMode.RendezvousSuspended || Mode == SavedFlightMode.FollowingSuspended || Mode == SavedFlightMode.ApproachDockSuspended;
    internal SavedFlightMode SuspendedMode => IsCombinedApproach ? SavedFlightMode.ApproachDockSuspended : IsDocking ? SavedFlightMode.DockingSuspended : IsFollowing ? SavedFlightMode.FollowingSuspended : IsPursuit ? SavedFlightMode.RendezvousSuspended : SavedFlightMode.Suspended;
    internal SavedFlightMode ActiveMode => IsCombinedApproach ? SavedFlightMode.ApproachDock : IsDocking ? SavedFlightMode.Docking : IsFollowing ? SavedFlightMode.Following : IsPursuit ? SavedFlightMode.Rendezvous : SavedFlightMode.Active;
    ...
    internal bool Valid => ObjectStateStore.SafeValue(ConsoleId) && ObjectStateStore.SafeValue(ModuleId) &&
        ...
        Enum.IsDefined(typeof(SavedFlightMode), Mode) && (!IsCombinedApproach ||
            (ObjectStateStore.SafeValue(OwnPort) && ObjectStateStore.SafeValue(TargetPort) && ArrivalMS == 0)) && (!IsPursuit || ArrivalMS == 0) && (!IsDocking ||
    ...
    internal Dictionary<string, string> Encode()
    {
        ...
        if (IsDocking || IsCombinedApproach) { fields["ownPort"] = OwnPort; fields["targetPort"] = TargetPort; }
        return fields;
    }
