...
{
    public static Dictionary<string,string> Save(string input,string filter,double energy,bool metered=false)
    {
        ...
        if(metered) d["mode"]="metered-v1";
        foreach(var value in d.Values)if(!ObjectStateStore.SafeValue(value))throw new ArgumentException("Unsafe treatment identity.");
        return d;
    }
    public static (string Input,string Filter,double Energy,bool Metered) Read(IReadOnlyDictionary<string,string>d)
    {
        bool metered=d.TryGetValue("mode",out var mode);
        if(d.Count!=(metered?4:3)||metered&&mode!="metered-v1")throw new ArgumentException("Unknown treatment fields.");
        string input=d["input"]=="none"?"":d["input"],filter=d["filter"]=="none"?"":d["filter"];
        double energy=double.Parse(d["energy"],CultureInfo.InvariantCulture);_=Save(input,filter,energy,metered);return(input,filter,energy,metered);
    }
}
